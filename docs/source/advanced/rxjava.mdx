...
Single<ApolloResponse<MyQuery.Data>> queryResponse = Rx2Apollo.rxSingle(queryCall);
queryResponse.subscribe( /* ... */ );

// Mutation
ApolloMutationCall<MyMutation.Data> mutationCall = client.mutate(new MyMutation("my-parameter"));
Single<ApolloResponse<MyMutation.Data>> mutationResponse = Rx2Apollo.rxSingle(mutationCall);
...
Flowable<ApolloResponse<MySubscription.Data>> subscriptionResponse = Rx2Apollo.rxFlowable(subscriptionCall);
...
ApolloStore apolloStore = ClientCacheExtensionsKt.getApolloStore(client);
